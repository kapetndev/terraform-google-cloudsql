...
resource "google_sql_database_instance" "replicas" {
  ...
  name                 = "${coalesce(var.descriptive_name, random_id.cluster_name[0].hex)}-${each.key}"
  project              = var.project_id
  ...
  settings {
    ...
    ip_configuration {
      allocated_ip_range = try(
        coalesce(
          each.value.network_config.connectivity.psa_config.allocated_ip_range,
          var.network_config.connectivity.psa_config.allocated_ip_range
        ),
        null
      )
      ...
      private_network = try(
        coalesce(
          each.value.network_config.connectivity.psa_config.private_network,
